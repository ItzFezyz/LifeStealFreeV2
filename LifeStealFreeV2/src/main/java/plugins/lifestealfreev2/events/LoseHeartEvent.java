...
import static plugins.lifestealfreev2.items.HeartItem.material;
...
public class LoseHeartEvent implements Listener {
    ...
    public void loseHeart(LoseHeartCustomEvent event){
        ItemStack Heart = new ItemBuilder(Material.matchMaterial(material.toUpperCase())).setTitle(IridiumColorAPI.process(HeartItem.name)).setLore(IridiumColorAPI.process(HeartItem.lore.toString())).build();
        ...
        if (event.getKiller() != null) {
            ...
            if (plugin.getConfig().getString("KillType").equals("DROP")) {
                ...
                HeartItem.getHeart().setAmount(1);
